...
# The script is used for alignment of monophones
# data=data/train, lang=data/lang, srcdir=exp/mono, align-dir=exp/mono_ali
data=$1
lang=$2
srcdir=$3
dir=$4
...
# oov.int contains the number of oov words
oov=`cat $lang/oov.int` || exit 1;
mkdir -p $dir/log
# writes the num_jobs into exp/mono_ali/num_jobs
echo $nj > $dir/num_jobs
# sdata => source-data, refers to the directory split4
sdata=$data/split$nj
#### no idea why the next line is? probably if we want to specify some kind
# of frame-splicing options
splice_opts=`cat $srcdir/splice_opts 2>/dev/null` # frame-splicing options.
# copying the above file to exp/mono_ali/
cp $srcdir/splice_opts $dir 2>/dev/null # frame-splicing options.
# cmvn : cepstral mean and variance normalization, it is empty atm
cmvn_opts=`cat $srcdir/cmvn_opts 2>/dev/null`
# copy it to exp/mono_ali
cp $srcdir/cmvn_opts $dir 2>/dev/null # cmn/cmvn option.
# similar as above, probably for delta coeff
delta_opts=`cat $srcdir/delta_opts 2>/dev/null`
cp $srcdir/delta_opts $dir 2>/dev/null
# -ot is older than, so it checks
# sdata should exists and $data/feats.scp should be older than $sdata
# split_data : doing per speaker, first arg is data_dir, second is num_to_split
# it basically splits the training data into 4
[[ -d $sdata && $data/feats.scp -ot $sdata ]] || split_data.sh $data $nj || exit 1;
# checks if the phones are existing
utils/lang/check_phones_compatible.sh $lang/phones.txt $srcdir/phones.txt || exit 1;
# copying relevan info
