...
#	# for x in train dev; do
#	trd=/home/arktheshadow/Softwares/Kaldi_asr/kaldi/egs/assgn2_1/recipe/data/train
#	x=train
#	# steps/make_mfcc.sh --nj $nj --cmd "$train_cmd" data/$x exp/make_mfcc/$x mfcc
#	steps/make_mfcc.sh --nj $nj --cmd "$train_cmd" $trd exp/make_mfcc/$x mfcc
#	# steps/compute_cmvn_stats.sh data/$x exp/make_mfcc/$x mfcc
#		steps/compute_cmvn_stats.sh $trd exp/make_mfcc/$x mfcc
#   # done
# fi
...
if [ $stage -le 4 ]; then
  ...
  steps/align_si.sh --nj $nj --cmd "$train_cmd" \
		 $trd $datadir/lang $expdir/mono exp/mono_ali
  # steps/train_deltas.sh --boost-silence 1.25  --cmd "$train_cmd"  \
  #			 3200 20000 data/train data/lang exp/mono_ali exp/tri1
